...
3. `Link Usage Plot`
   ...
   The **Link Usage Plot** displays how many packets traversed each satellite link during the simulation. It reveals the traffic load distribution in the satellite network.
...
- **FromDevice**: The city that sends the packet.
- **ToDevice**: The city that receives the packet.
- **Latency_ms**: The time (in milliseconds) it takes for a packet to transfer from source to destination.
- **Avg_Hop**: The average number of hops that packets take from source ground station to the destination ground station.
- **Stretch_factor**: The ratio between the average latency and the latency based on geodesic distance divided by the speed of light.
...
- **SatelliteA**: The name of one end of the satellite link.
- **SatelliteB**: The name of the other satellite link.
- **UsageCount**: The counts of packets traversed this satellite link during the simulation.
---
You can run the Analyze.py with the following commands:
> python3 log_analyzer/analyze.py [SimulationSummary file] [ground_stations location]
