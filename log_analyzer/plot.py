...
def hop_count_stretch_factor_effective_latency_cdf_plot(overall: pd.DataFrame,results_dir: str) -> None:
    ...
    plt.plot(hop_x, hop_cdf, linestyle='--', label="Hop Count CDF", color='green')
    ...
    plt.savefig(f"{results_dir}/hop_stretch_effective_cdf_plot.png")
    print(f'Stretch Factor, Hop Count, and Effective Latency Factor CDF plot saved as {results_dir}/hop_stretch_effective_cdf_plot.png')
...
def generate_plots(overall: pd.DataFrame, results_dir: str) -> None:
    ...
    hop_count_stretch_factor_effective_latency_cdf_plot(overall, results_dir)
    RTT_cdf_plot(overall, results_dir)
    satellite_usage_plot(results_dir)

def combine_hop_count_stretch_factor_effective_latency(overalls, results_dir, combination_name: str) -> None:
    plt.figure(dpi=300)
    ...
    print(f'Combined Stretch Factor, Hop Count, and Effective Latency Factor CDF plot saved as {results_dir}/combined_hop_stretch_effective_cdf_plot.png')
...
def combine_RTT(overalls, results_dir, combination_name: str) -> None:
    ...
    print(f'Combined RTT and CDF plot saved as {results_dir}/combined_RTT_cdf_plot.png')
...
def combine_overalls(overalls: list, combination_folder: str, combination_name: str) -> None:
    ...
    combine_hop_count_stretch_factor_effective_latency(overalls_with_colors, combination_folder, combination_name)
