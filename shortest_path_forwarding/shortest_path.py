...
def calculate_shortest_paths(node_writers, node_files, total_time, time_step, graph_generator):
    for time_stamp in range(0, total_time + 1, time_step):
        ...
        calculate_shortest_path_hops(node_writers, time_stamp, graph, graph_generator.constellation_name)
...
def dijkstra_shortest_path_algorithm(distance_file_name):
    ...
    graph_generator = dfg.GraphGenerator(distance_csv_dataframe, constellation_name, dfg.NXGraphBuilder())
...
def dijkstra_grid_plus_shortest_path_algorithm(distance_file_name):
    ...
    grid_plus_graph_generator = dfg.GridPlusGraphGenerator(distance_csv_dataframe, constellation_name, dfg.NXGraphBuilder(), number_of_orbits, number_of_satellites_per_orbit)
...
def dijkstra_static_topology_shortest_path_algorithm(distance_file_name, topology_file_name):
    ...
    static_topology_graph_generator = dfg.StaticTopologyGraphGenerator(distance_csv_dataframe, constellation_name, dfg.NXGraphBuilder(), topology_file_name)
...
def dijkstra_dynamic_topology_shortest_path_algorithm(distance_file_name, topology_file_name):
    ...
    dynamic_topology_graph_generator = dfg.DynamicTopologyGraphGenerator(distance_csv_dataframe, constellation_name, dfg.NXGraphBuilder(), topology_file_name)
